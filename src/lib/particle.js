const Particle = {
	// 根据传入的config初始化画布
	canvasInit:function(canvasConfig,elementname){
		...
		var canvas = {
			...
			config: {
				...
				stroke: canvasConfig.stroke || "144,174,207",
		...
		Particle.canvasSize(canvas.element,elementname);
		...
		setInterval(function(){
			Particle.drawPoint(canvas);
		}, 40);
	},

	// 设置canvas大小
	canvasSize:function(canvas,elementname){
		// 获取窗口的宽高
	...
	drawPoint:function(canvas){
		...
		for(var i = 0, len = canvas.config.count; i < len; i++){
			if(canvas.points.length !== canvas.config.count){
				...
				// 处理球的速度和位置，并且做边界处理
				point =	Particle.borderPoint(canvas.points[i], canvas);
			}
			context.fillRect(point.x - canvas.config.width / 2, point.y - canvas.config.height / 2, canvas.config.width, canvas.config.height);

			canvas.points[i] = point;
		}
		Particle.drawLine(context, canvas, canvas.mouse, dist);
		context.closePath();
	},
	...
	// 画线
	drawLine:function(context, canvas, mouse, dist){
